...
export function predictionToItem(p: Prediction): Item {
  return {
    ...
    home: p.home,
    away: p.away,
    firstTeam: p.firstTeam ?? null,
    firstScorerId: p.firstScorerId ?? null,
    firstScorerName: p.firstScorerName ?? null,
    points: p.points,
    exact: p.exact ?? false,
    correctOutcome: p.correctOutcome ?? false,
    joker: p.joker ?? false,
    createdAt: p.createdAt,
    updatedAt: p.updatedAt,
  };
...
export function predictionFromItem(item: Item): Prediction {
  return {
    ...
    firstTeam: (item.firstTeam ?? null) as Prediction['firstTeam'],
    firstScorerId: (item.firstScorerId ?? null) as string | null,
    firstScorerName: (item.firstScorerName ?? null) as string | null,
    points: item.points as Prediction['points'],
    exact: (item.exact ?? false) as boolean,
    correctOutcome: (item.correctOutcome ?? false) as boolean,
